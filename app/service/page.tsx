...
export default async function ServicePage({searchParams}:{searchParams?:Promise<{locale?:string}>}){
  ...
  const sections=zh?[
    ...
    ["付款问题与退款联系",`取消或待确认的付款不会解锁报告。对于重复付款、无法交付或其他订单问题，请联系 ${destinySupportEmail}，并提供账号邮箱和订单号，不要发送银行卡信息。退款或支付撤销确认后，对应订单的完整报告访问将撤销。`],
    ...
    ["Payment issues and refund support",`Cancelled or pending payments do not unlock reports. For duplicate charges, delivery failures or other order issues, contact ${destinySupportEmail} with your account email and order number. Do not send card information. Once a refund or reversal is confirmed, access granted by that order is revoked.`],
