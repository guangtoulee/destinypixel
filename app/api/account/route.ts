...
export async function GET() {
  try {
    ...
    const paid = new Set(orders.filter(o=>o.status==="completed" && (o.mode==="live" || (o.mode==="sandbox" && paypalMode()==="sandbox" && (process.env.VERCEL_ENV!=="production" || isAdmin)))).map(o=>o.report_id));
    return privateJson({
      member: {id:member.id,email:member.email,name:member.name,plan:member.plan,isAdmin}, ...readiness,
      reports:access.map(a=>({id:a.report_id,title:a.reports?.birth_records?.name || "Birth report",locale:a.reports?.birth_records?.locale || "en",createdAt:a.created_at,access:!paidReportsEnabled()||isAdmin||paid.has(a.report_id)?"full":"basic",status:a.reports?.status || "ai_pending"})),
