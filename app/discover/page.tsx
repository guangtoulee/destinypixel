...
export default async function Discover({searchParams}:Props){
 ...
 return <main className={styles.page} lang={journalLanguageTags[locale]} data-server-localized><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(schema).replace(/</g,"\\u003c")}}/><header className={styles.header}><a className={styles.brand} href={home}><span/>DestinyPixel</a><nav aria-label="Language">{discoveryLocales.map(l=><a key={l} href={discoveryHref(l)} aria-current={l===locale?"page":undefined}>{({en:"EN",zh:"简体","zh-TW":"繁體",ru:"RU"})[l]}</a>)}</nav></header><div className={styles.hero}><section className={styles.intro}><p className={styles.label}>{copy.eyebrow}</p><h1>{copy.heading}</h1><p className={styles.lead}>{copy.intro}</p><ul className={styles.features}>{copy.features.map(x=><li key={x}><Check size={14}/>{x}</li>)}</ul><div className={styles.preview}>{["癸卯","丙午","辛巳"].map(p=>{const c=cards.find(x=>x.pillar===p)!;return <figure key={p}><Image src={c.image} alt={c.name} width={896} height={1200} sizes="(max-width:650px) 28vw, 130px"/><figcaption>{c.name}</figcaption></figure>})}</div></section><BirthdayCardFinder cards={cards} copy={copy.finder} locale={locale}/></div><p style={{maxWidth:1120,margin:"24px auto",padding:"0 24px",textAlign:"center"}}><a href={pillarLibraryHref(locale)} style={{color:"#684688",textDecoration:"underline",textUnderlineOffset:4}}>{pillarLibraryCopy(locale).browse}</a></p><section className={styles.steps}><h2>{copy.nextTitle}</h2><div>{copy.steps.map((step,i)=><article key={step.title}><span>0{i+1}</span><h3>{step.title}</h3><p>{step.body}</p></article>)}</div></section><section className={styles.faq}><h2>{copy.faqTitle}</h2><div>{copy.faqs.map(x=><details key={x.q}><summary>{x.q}</summary><p>{x.a}</p></details>)}</div></section><ProductSearchContent product="discovery" locale={locale} /><footer className={styles.footer}><a href={home}>DestinyPixel</a><a href={`/journal${locale==="en"?"":`?locale=${locale}`}`}>{copy.journal}<ArrowRight size={14}/></a></footer></main>;
}
