...
export function generateStaticParams() {
  return getIndexablePromptItems()
    .filter((item) => !isPromptArticle(item))
    .map((item) => ({ id: item.id }));
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title,
    description,
    keywords: [...item.tags, ...item.modelHints, item.category, "AI Prompt 案例", "提示词拆解"],
    ...
    openGraph: {
      ...
      images: item.imageUrl ? [{ url: item.imageUrl, alt: title }] : ["/opengraph-image"],
    },
    twitter: { card: "summary_large_image", title, description, images: item.imageUrl ? [item.imageUrl] : ["/opengraph-image"] },
    robots: { index, follow: true, googleBot: { index, follow: true, "max-image-preview": "large", "max-snippet": -1 } },
  };
}
...
export default async function PromptCasePage({ params }: PageProps) {
  ...
  const schema = {
    ...
    keywords: [...item.tags, ...item.modelHints].join(", "),
