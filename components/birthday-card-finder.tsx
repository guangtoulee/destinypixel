...
export default function BirthdayCardFinder({locale,copy,cards}:{locale:ReportLocale;copy:DiscoveryCopy["finder"];cards:DayPillarCard[]}) {
 ...
 return <section className={styles.finder} id="find-your-card" aria-label={copy.label}>
  ...
  <div ref={resultRef} tabIndex={-1} className={styles.result} aria-live="polite">
   {card?<><p className={styles.label}>{copy.result}</p><div className={styles.revealed}><Image src={card.image} alt={card.name} width={896} height={1200} sizes="(max-width:650px) 160px, 190px"/><div><h2 lang={locale==="ru"?"en":undefined}>{card.name}</h2><p>{copy.note}</p></div></div><DayPillarReading insight={card.insight} locale={locale} growth={card.growth}/><div className={styles.resultActions}><button type="button" onClick={()=>void shareCard()}><Copy size={14}/>{copy.share}</button><a href={`${home}#report`}>{copy.full}<ArrowRight size={14}/></a></div>{card.pillar==="甲子"&&<a className={styles.storyLink} href={`/journal/jia-zi-day-pillar${locale==="en"?"":`?locale=${locale}`}`}>{copy.read}<ArrowRight size={15}/></a>}{share&&<div className={styles.share}><p role="status">{shareMessage}</p><input aria-label={copy.share} value={share} readOnly onFocus={e=>e.target.select()}/></div>}</>:<div className={styles.empty}><Sparkles size={25}/><h2>{copy.prompt}</h2><p>{copy.promptBody}</p></div>}
  </div>
 </section>;
}
