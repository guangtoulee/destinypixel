...
const copy = {
  en: {
    ...
    next: "BRING THE REST OF YOUR STORY", nextTitle: "Your day card is the beginning.", nextBody: "Continue on the main site with your birth time and city. These details add depth to your personal birth map. A result near midnight can shift to the adjacent day.", nextAction: "Continue to my birth map", nextNote: "You will enter your details again on the main form. This page does not transfer or store them.",
  ...
  zh: {
    ...
    next: "把出生坐标补充完整", nextTitle: "日柱卡，只是故事的开头。", nextBody: "回到主站，补充出生时间与城市。进一步校准日柱，探索更完整的性格与人生图谱。午夜附近的结果可能移到相邻一天。", nextAction: "继续查看出生图谱", nextNote: "需要在主站表单重新填写资料；本页不会传递或储存出生信息。",
...
export default function DayPillarExperience({ locale, cards, initialPillar, isShared }: { locale: DayPillarLocale; cards: DayPillarCard[]; initialPillar: string; isShared: boolean }) {
  ...
  async function reveal(event: FormEvent<HTMLFormElement>) {
    ...
    if (busy) return;
    const date = String(new FormData(event.currentTarget).get("birthDate") ?? "");
    setBusy(true); setError("");
    try {
      const { calculateDateDayPillar } = await import("@/lib/day-pillar");
      const result = calculateDateDayPillar(date, localToday());
  ...
  return <main className={styles.page} lang={locale === "zh" ? "zh-Hans" : "en"}>
    ...
    <div className={styles.mainGrid}>
      <section className={styles.introduction}>
        ...
        <form className={styles.form} onSubmit={reveal} noValidate>
          ...
          <input id="day-pillar-date" name="birthDate" type="date" onInput={() => setError("")} min="1800-01-01" max="2100-12-31" autoComplete="off" aria-invalid={Boolean(error)} aria-describedby={error ? "day-pillar-error day-pillar-convention" : "day-pillar-convention"} required />
      ...
      <section className={styles.result} ref={resultRef} tabIndex={-1} aria-label={text[mode]}>
        <div className={styles.resultHeading}><p className={styles.eyebrow} aria-live="polite">{text[mode]}</p><span>{locale === "zh" ? card.pillar : "60 CHARACTERS"}</span></div>
        <div className={styles.stage} onPointerMove={moveLight} onPointerLeave={resetLight}>
          ...
          <div className={styles.cardSurface} ref={artRef} data-tilted={tilted}><Image src={card.image} alt={locale === "zh" ? `${card.pillar} · ${card.name}` : card.name} width={896} height={1200} priority sizes="(max-width: 820px) 86vw, 350px" /><div className={styles.glint} aria-hidden="true" /></div>
    ...
    <footer className={styles.footer}><div><strong>DestinyPixel</strong><p>{text.footer}</p></div><Link href={locale === "zh" ? "/journal/what-is-a-day-pillar?locale=zh" : "/journal/what-is-a-day-pillar"}>{locale === "zh" ? "日柱是什么？" : "What is a Day Pillar?"}</Link><Link href={home}>{text.home}</Link><a href={destinySupportHref}>{text.contact}</a></footer>
  </main>;
}
