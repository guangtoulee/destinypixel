...
export default function ReportUnlock({ reportId, locale, isMember, claimable, offer }: { reportId: string; locale: ReportLocale; isMember: boolean; claimable: boolean; offer: Offer }) {
  ...
  return <><div className={styles.saveBar} data-export-hide><div><p>{saved ? text.saved : text.saveIntro}</p>{saveError && <p className={styles.error} role="alert">{saveError}</p>}</div>{saved ? <Link className={styles.textLink} href={accountPath}><Check size={15} aria-hidden="true" />{text.account}</Link> : <button type="button" className={styles.saveButton} onClick={saveReport} disabled={saving}>{saving ? <Loader2 size={15} className={styles.spin} aria-hidden="true" /> : <Save size={15} aria-hidden="true" />}{saving ? text.saving : text.save}</button>}</div><section id="unlock-full-report" className={styles.unlock} aria-labelledby="full-report-title" data-export-hide>
    ...
    <div className={styles.chapterPreview}>{(contentLocale(locale) === "zh" ? [{title:"同样的天赋，为什么发挥不同？",body:"结合另外三柱、五行与行星位置，展开你的核心、外在形象和深层自我。"},{title:"事业与感情里，哪些模式值得留意？",body:"从完整出生图谱出发，继续阅读工作、关系与成长的具体解读。"},{title:"这一年，可以怎样安排节奏？",body:"打开年运月令，阅读年度概览与十二个月度篇章，作为规划与反思的参考。"}] : locale === "ru" ? [{title:"Почему один талант проявляется по-разному?",body:"Остальные столпы, стихии и планеты дополняют главы о внутреннем мире и внешнем образе."},{title:"Какие темы повторяются в работе и отношениях?",body:"Продолжите чтение о карьере, близости и росте с учетом полной карты рождения."},{title:"Как осмыслить ритм этого года?",body:"Откройте годовой обзор и двенадцать ежемесячных глав для размышления и планирования."}] : [{title:"Why does the same strength show up differently?",body:"The other pillars, elements and planetary positions add context to your core, outer persona and deeper self."},{title:"What patterns deserve attention in work and love?",body:"Continue into career, relationship and growth chapters informed by your complete birth map."},{title:"How could you reflect on this year?",body:"Open an annual overview and twelve monthly chapters as prompts for reflection and planning."}]).map((item,index)=><article key={item.title}><span><LockKeyhole size={13} aria-hidden="true"/>0{index+1}</span><h3>{item.title}</h3><p>{item.body}</p></article>)}</div>
